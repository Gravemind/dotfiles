...
function draw_bar(cr, bar)

	cairo_rectangle(cr, bar.x, bar.y, bar.w, bar.h)
	cairo_set_source_rgba(cr, rgb_to_r_g_b(bar.fg_colour, bar.fg_alpha))
	cairo_fill(cr)

	-- cairo_set_line_width(cr, bar.h)
	-- cairo_set_line_cap(cr, CAIRO_LINE_CAP_BUTT)
	-- cairo_set_source_rgba(cr, rgb_to_r_g_b(bar.fg_colour, bar.fg_alpha))
	-- cairo_move_to(cr, bar.x, bar.y)
	-- cairo_line_to(cr, bar.x + bar.w, bar.y)
	-- cairo_stroke(cr)
end
...
function conky_clock_rings()
	...
	--Check that Conky has been running for at least 3s
